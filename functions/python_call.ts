...
const salary_path = path.join(__dirname, 'salary_recalculation.py');
...
export async function salary(): Promise<void> {
    return new Promise((resolve, reject) => {
        // Spawn a new child process to run the Python script
        const Process = spawn('./venv/bin/python3', [salary_path]);
